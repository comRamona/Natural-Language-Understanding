...
if __name__ == "__main__":
    ...
    if mode == "train-lm":
        ...
        ##########################
        import logging
        logging.basicConfig(filename="train-lm-seed2.log", level=logging.INFO)
        logging.root.level = logging.INFO

        for hdim in [25, 50, 75]:
            for lr in [0.05, 0.5, 0.1]:
                for lookback in [0, 2, 5]:
                    for anneal in [5, 10]:
                        np.random.seed(2018)
                        rnn = RNN(vocab_size, hdim, vocab_size)
                        run_loss = rnn.train(X_train, D_train, X_dev, D_dev, learning_rate=lr, back_steps=lookback, anneal=anneal,epochs=10)
                        ...
                        log_d = "hdim:{} , lr: {} lookback: {} anneal: {} \n run_loss: {:.3f} unadjusted: " \
                        "{:.3f} basic adjusted: {:.3f} normal adjusted: {:.3f}".format(hdim,lr,lookback,anneal, run_loss,
                            np.exp(run_loss), np.exp(basic_adjusted_loss), np.exp(normal_adjusted_loss))
        ...
        rnn = RNN(vocab_size, hdim, vocab_size)
        run_loss = rnn.train(X_train, D_train, X_dev, D_dev, learning_rate=lr, back_steps=lookback, anneal=10)
    ...
    if mode == "train-full-lm":
        ...
        rnn = RNN(vocab_size, hdim, vocab_size)
        run_loss = rnn.train(X_train, D_train, X_dev, D_dev, learning_rate=lr, back_steps=lookback, anneal=anneal)
        ...
        import logging
        logging.basicConfig(filename="train-lm-results.log", level=logging.INFO)
        logging.root.level = logging.INFO
        print("Mean loss:{:.3f}".format(test_loss))
        print("Unadjusted perplexity: %.03f" % np.exp(test_loss))
        print("Basic adjusted for missing vocab: %.03f" % np.exp(basic_adjusted_loss))
        print("Normal adjusted for missing vocab: %.03f" % np.exp(normal_adjusted_loss))
        logging.info("Mean Loss: {:.3f} Unadj perplexity: {:.3f} Basic adjusted: {:.3f} Normal adjusted: {:.3f}".format(test_loss,
        np.exp(test_loss), np.exp(basic_adjusted_loss), np.exp(normal_adjusted_loss)))
        from os.path import join
        np.save(join(rnn_folder, 'rnn.U.npy'), rnn.U)
        np.save(join(rnn_folder, 'rnn.V.npy'), rnn.V)
        np.save(join(rnn_folder, 'rnn.W.npy'), rnn.W)
    ...
    if mode == "train-np":
        ...
        logging.basicConfig(filename="train-np-seed2.log", level=logging.INFO)
        logging.root.level = logging.INFO
        logging.info("Train-np experiments")
        for hdim in [25, 50, 75]:
            for lr in [0.1, 0.5, 0.05]:
                for lookback in [0, 2, 5]:
                    for anneal in [5, 10]:
                      for epochs in [10,20]:
                          np.random.seed(2018)
                          rnn = RNN(vocab_size, hdim, vocab_size)
                          run_loss = rnn.train_np(X_train, D_train, X_dev, D_dev, learning_rate=lr, back_steps=lookback, anneal=anneal,epochs=epochs)

                          acc = sum([rnn.compute_acc_np(X_dev[i], D_dev[i]) for i in range(len(X_dev))]) / len(X_dev)

                          log_d = "hdim:{} , lr: {} lookback: {} anneal: {} epochs: {} -- Accuracy: {:.3f} Loss: {:.3f}".format(hdim,lr,lookback,anneal, epochs, acc, run_loss)

                          print(log_d)
                          logging.info(log_d)


    if mode == "train-full-np":
        '''
        starter code for parameter estimation.
        change this to different values, or use it to get you started with your own testing class
        '''
        ...
        loss = sum([rnn.compute_loss_np(X_test[i], D_test[i]) for i in range(len(X_test))]) / loss_sum
        print("Loss: %.03f" % loss)
        import logging
        logging.basicConfig(filename="train-np-results.log", level=logging.INFO)
        logging.root.level = logging.INFO
        logging.info("Loss: {:.3f} Acc: {:.3f}".format(acc, loss))
